...
describe('GET /hotels', () => {
  ...
  it('should respond with status 401 if there is no session for this token', async () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 402 when ticket was not paid yet', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
    ...
    it('should respond with status 200 when hotels is returned', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType({ isRemote: false, includesHotel: true });
      await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      await createHotel({ name: 'Winston' });
...
describe('GET /hotels/:hotelId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 200 when hotel with rooms is returned', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketType({ isRemote: false, includesHotel: true });
      await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const hotel = await createHotel({ name: 'Winston' });
